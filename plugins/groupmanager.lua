...
if not tonumber(data.sender_user_id) then return false end
  ...
  if not lang then
    return tdbot.sendMessage(data.chat_id, "", 0, "_Group is not added_", 0, "md")
else
    return tdbot.sendMessage(data.chat_id, "", 0, "_گروه به لیست گروه های مدیریتی ربات اضافه نشده است_", 0, "md")
     end
  end
...
if administration[tostring(arg.chat_id)]['whitelist'][tostring(data.id)] then
   if not lang then
    return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _is already in_ *white list*", 0, "md")
else
    return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id.."* *از قبل در لیست سفید بود*", 0, "md")
      end
   end
administration[tostring(arg.chat_id)]['whitelist'][tostring(data.id)] = user_name
    save_data(_config.moderation.data, administration)
   if not lang then
    return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _has been added to_ *white list*", 0, "md")
   else
    return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id.."* *به لیست سفید اضافه شد*", 0, "md")
   end
end
tdbot_function ({
    _ = "getUser",
...
if not administration[tostring(arg.chat_id)]['whitelist'][tostring(data.id)] then
    if not lang then
    return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _is not in_ *white list*", 0, "md")
    else
    return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id.."* *از قبل در لیست سفید نبود*", 0, "md")
   end
  end
administration[tostring(arg.chat_id)]['whitelist'][tostring(data.id)] = nil
    save_data(_config.moderation.data, administration)
   if not lang then
    return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _has been removed from_ *white list*", 0, "md")
   else
    return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id.."* *از لیست سفید حذف شد*", 0, "md")
   end
end
tdbot_function ({
    _ = "getUser",
...
if administration[tostring(arg.chat_id)]['owners'][tostring(data.id)] then
    if not lang then
    return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _is already a_ *group owner*", 0, "md")
   else
    return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id.."* *از قبل صاحب گروه بود*", 0, "md")
      end
   end
administration[tostring(arg.chat_id)]['owners'][tostring(data.id)] = user_name
    save_data(_config.moderation.data, administration)
   if not lang then
  return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _is now the_ *group owner*", 0, "md")
   else
  return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id.."* *به مقام صاحب گروه منتصب شد*", 0, "md")
   end
end
tdbot_function ({
    _ = "getUser",
...
if administration[tostring(arg.chat_id)]['mods'][tostring(data.id)] then
   if not lang then
    return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _is already a_ *moderator*", 0, "md")
else
    return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id.."* *از قبل مدیر گروه بود*", 0, "md")
      end
   end
administration[tostring(arg.chat_id)]['mods'][tostring(data.id)] = user_name
    save_data(_config.moderation.data, administration)
   if not lang then
    return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _has been_ *promoted*", 0, "md")
   else
    return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id.."* *به مقام مدیر گروه منتصب شد*", 0, "md")
   end
end
tdbot_function ({
    _ = "getUser",
...
if not administration[tostring(arg.chat_id)]['owners'][tostring(data.id)] then
   if not lang then
return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _is not a_ *group owner*", 0, "md")
   else
return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* *از قبل صاحب گروه نبود*", 0, "md")
      end
   end
administration[tostring(arg.chat_id)]['owners'][tostring(data.id)] = nil
    save_data(_config.moderation.data, administration)
   if not lang then
return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _is no longer a_ *group owner*", 0, "md")
    else
return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id.."* *از مقام صاحب گروه برکنار شد*", 0, "md")
   end
end
tdbot_function ({
    _ = "getUser",
...
if not administration[tostring(arg.chat_id)]['mods'][tostring(data.id)] then
    if not lang then
    return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _is not a_ *moderator*", 0, "md")
    else
    return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id.."* *از قبل مدیر گروه نبود*", 0, "md")
   end
  end
administration[tostring(arg.chat_id)]['mods'][tostring(data.id)] = nil
    save_data(_config.moderation.data, administration)
   if not lang then
    return tdbot.sendMessage(arg.chat_id, "", 0, "_User_ "..user_name.." *"..data.id.."* _has been_ *demoted*", 0, "md")
   else
    return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر_ "..user_name.." *"..data.id.."* *از مقام مدیر گروه برکنار شد*", 0, "md")
   end
end
tdbot_function ({
    _ = "getUser",
...
local cmd = arg.cmd
  ...
  if not lang then
    return tdbot.sendMessage(arg.chat_id, "", 0, "_Group is not added_", 0, "md")
else
    return tdbot.sendMessage(arg.chat_id, "", 0, "_گروه به لیست گروه های مدیریتی ربات اضافه نشده است_", 0, "md")
     end
  end
...
username = 'ندارد'
    ...
    if not lang then
  return tdbot.sendMessage(arg.chat_id, "", 0, "_User not founded_", 0, "md")
   else
  return tdbot.sendMessage(arg.chat_id, "", 0, "_کاربر یافت نشد_", 0, "md")
    end
  end
...
-- vardump(data)
   if data.photos[0] then
       if not lang then
        tdbot.sendPhoto(msg.to.id, msg.id, data.photos[0].sizes[1].photo.persistent_id, 0, {}, 0, 0, 'Chat ID : '..msg.to.id..'\nUser ID : '..msg.from.id, 0, 0, 1, nil, dl_cb, nil)
       elseif lang then
   ...
   local lang = redis:get(hash)
    local administration = load_data(_config.moderation.data) 
       ...
       return tdbot.sendMessage(msg.to.id, msg.id, 1, "*Newlink Created*", 1, 'md')
        elseif lang then
       return tdbot.sendMessage(msg.to.id, msg.id, 1, "_لینک جدید ساخته شد_", 1, 'md')
            end
				end
			end
...
*!unmute* `[gif | photo | document | sticker | keyboard | video | video_note | text | forward | location | audio | voice | contact | all]`
_If This Actions Unlock, Bot Not Delete Them_
...
*باصدا* `[همه | تصاویر متحرک | عکس | اسناد | برچسب | صفحه کلید | فیلم | فیلم سلفی | متن | نقل قول | موقعیت | اهنگ | صدا | مخاطب | کیبورد شیشه ای|بازی|خدمات تلگرام]`
_If This Actions Unlock, Bot Not Delete Them_
	...
	local secchk = true
		...
		if secchk then
			...
			if not lang then
				...
				return tdbot.sendMessage(msg.to.id, 0, 1, '_لطفا برای کارکرد کامل دستورات، ربات را به مدیر گروه ارتقا دهید._', 1, "md")
			end
		end
	end, nil)
 end
